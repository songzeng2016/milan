...
Page({
  ...
  submit: function (e) {
    ...
    var { form, user } = this.data;
    form.sponsors = form.sponsorList.filter(item => !!item).join(',');
    form.undertakers = form.undertakerList.filter(item => !!item).join(',');
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (msg == "success") {
          ...
          form.exhibitionId = '';
          form.exhibitionName = '';
          that.setData({ form: form, isOk: false });
          //保存图片
          var activityId = activity.id;
          if (activityId > 0) {
            that.uploadActivityImg(activityId);
          }
          that.setData({ activityId: activityId || 0 });
        } else {
          wx.hideToast();
          wx.showToast({
            title: '发布活动失败',
